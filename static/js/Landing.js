...
function userLoadings(wx,baseUrl,basePath,callBack){
  ...
  wx.setStorageSync('tnop257',0)
  wx.setStorageSync('tnop258',0)
      ...
      success: function(rs) {
        ...
        wx.getUserInfo({ //获取用户相关信息
          ...
          success: (userRes) => {
            console.log('登陆')
            console.log(userRes)
            wx.setStorageSync('userinfos', userRes.userInfo)
            wx.showToast({ title: '登陆中', icon: 'loading' })
            wx.getSystemInfo({ //获取用户 手机 系统信息
              success: function(res){
                wx.setStorageSync('city', JSON.parse(userRes.rawData).city)
    ...
    wx.login({
      success: function(rs) {  //获取用户信息
        ...
        wx.getUserInfo({
          lang:'zh_CN',
          success: (userRes) => {
            //JSON.parse(userRes.rawData).province+','+
            console.log('获取用户信息')
            ...
            wx.getSystemInfo({
              success: function(res){
                ...
                let $da = {code:data.code, iv:data.iv, encryptdata:data.encryptdata, inviter:parseInt(data.inviter),
                  device:data.device, version:data.version}
...
function loginInfo($da,baseUrl,basePath,callBack){
  ...
  wx.request({
    ...
    success (res) {
      console.log('------------------------登陆---------------------')
      console.log(res)
      let temp = wx.getStorageSync('userinfos')
      console.log(temp)
      temp.id = res.data.user.uid
      wx.setStorageSync('userinfos', temp)
